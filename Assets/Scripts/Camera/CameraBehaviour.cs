...
public class CameraBehaviour : MonoBehaviour
{
  ...
  //      BlurOk = true;
    ...
    public void SetTarget(GameObject tagrget)
    {
        Target =  tagrget;
   ...
   /*     if (CriticalHitEffect)
    ...
    void UpdateFov()
    {
        if (FovCameraOk == false)
        {
            ...
            if (CurrentFovTime >= 0)
                CurrentCameraFov = Mathfx.Hernite(FovStart, FovCameraEnd, CurrentFovTime / FovTime);
        }
    ...
    void UpdateSloMotion()
    {
        if (ShiftOk == false)
        {
            ...
            if (CurrentShiftTime >= 0)
                TimeScaleCurrent = Mathfx.Hernite(TimeScaleStart, TimeScaleEnd, CurrentShiftTime / ShiftTime);
        }
