...
public class Agent : MonoBehaviour, IEDable
{
    ...
    public Transform progressControlTF;
    ...
    public bool IsAlive { get { return GameObject.activeSelf && BlackBoard.Health > 0; } }
    //是否可见:渲染组件是否显示
    ...
    {
        ResourceManager.LoadAssetAsync("Assets/Game/Prefabs/UI/Role/ProgerssItem.prefab", typeof(GameObject), (obj) =>
        {
            GameObject _obj = Instantiate(obj, CanvasWorld) as GameObject;
            progressControlTF = _obj.transform;
            progressControl = _obj.transform.GetComponent<ProgressControl>();
            progressControl.UpdateProgress(1.0f);
        });
    }
    ...
    public void ReceiveDamage(Agent attacker, E_WeaponType byWeapon, float damage, AnimAttackData data)
    {
        ...
        if (IsKnockedDown)
        {
            ...
            // todo  暴击特效
            progressControl.UpdateProgress(0, GetSpeed(damage));
            progressControlTF.gameObject.SetActive(false);
        }
        else
        {
            ...
            if (IsAlive)
            {
                ...
                //地上出现一滩血
                SpriteEffectsManager.Instance.CreateBlood(Transform);// by chenyun 先屏蔽掉
                SpriteEffectsManager.Instance.ShowDemageText(Transform, damage);
                progressControl.UpdateProgress(BlackBoard.Health/ BlackBoard.MaxHealth, GetSpeed(damage));
            }
            else
            {
                WorldState.SetWSProperty(E_PropKey.E_EVENT, E_EventTypes.Died);
    ...
    public void ReceiveCriticalHit(Agent attacker, E_CriticalHitType type, bool effectOnly = false)
    {
        ...
        // todo  暴击特效
        //progressControl.UpdateProgress(0);
    ...
    protected IEnumerator FadeIn()
    {
        ...
        progressControlTF.gameObject.SetActive(true);
    }
