...
public class ComponentPlayer : MonoBehaviour, IActionHandle
{
    ...
    public Transform m_EasyTouchTrans;
    ...
    public void SetEasyTouch()
    {
        Debug.Log("Test...");
        m_EasyTouchTrans = GameObject.Find("EasyTouch").transform;
        m_EasyTouch = m_EasyTouchTrans.GetComponent<EasyTouch>();
    }

    public void SetTargetEx()
    {
        GameObject InGameCamera = GameObject.Find("InGameCamera");
        CameraControl cameraControl =  InGameCamera.GetComponent<CameraControl>();
        CameraBehaviour cameraBehaviour =  InGameCamera.GetComponent<CameraBehaviour>();
        cameraControl.SetTarget(gameObject);
        cameraBehaviour.SetTarget(gameObject);
    }
    ...
    void Update()
    {
        ...
        else
        {
           if(m_EasyTouch!=null)
           {
                 Vector3 nowPosition = m_EasyTouch.GetOffsetVector3(); //先屏蔽掉
                OnJoystickMove(nowPosition);
           }
    ...
    public void PlayerSkill_X()
    {
        CreateOrderAttack(E_AttackType.X);
    }
    public void PlayerSkill_O()
    {
       CreateOrderAttack(E_AttackType.O);
    }

    public void PlayerSkill_I()
    {
        CreateOrderDodge();
    }
    void OnInputMove()
    ...
    void OnJoystickMove(Vector3 move)
    {
        if (Math.Abs(move.x) <= 0.10 && Math.Abs(move.y) <= 0.10)
        {
            CreateOrderStop();
            return;
        }
        else
        {
            ...
            float mForce = Mathf.Clamp(Direction.magnitude, 0, 1);
            Direction.Normalize();

            if (Direction != Vector3.zero)
            {
                CreateOrderGoTo(Direction, mForce);
            }
            else
            {
                CreateOrderStop();
    ...
    void LateUpdate()
    {
        if (StepTime < Time.timeSinceLevelLoad)
        {
            if (Agent.BlackBoard.MotionType == E_MotionType.Run)
            {
                ...
                StepTime = Time.timeSinceLevelLoad + UnityEngine.Random.Range(0.25f, 0.28f);
            }
            else if (Agent.BlackBoard.MotionType == E_MotionType.Walk)
            {
                Agent.Sound.PlayStep();
                StepTime = Time.timeSinceLevelLoad + UnityEngine.Random.Range(0.40f, 0.43f);
            }
        }
        ...
        if(Agent.progressControlTF != null)
        {
            Agent.progressControlTF.position = new Vector3(transform.position.x, transform.position.y + 1.5f, transform.position.z + 1.0f);
        }
    }
